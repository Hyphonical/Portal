	...
	>
		...
		<div class="flex flex-col gap-3">
			<div class="flex flex-wrap items-center justify-between gap-3">
				...
				<div class="flex items-center gap-2">
					...
					>
						{{ cfg.mcServerHost
						}}<template v-if="cfg.mcServerPort && Number(cfg.mcServerPort) !== 25565"
						...
						<span class="ml-2 cursor-pointer" @click="copyIp" title="Copy IP">
							<svg
								...
								class="h-5 w-5 text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors"
								fill="none"
								viewBox="0 0 24 24"
								stroke="currentColor"
							>
		...
		<div class="flex items-center gap-3">
			...
			<NuxtLink
				...
				target="_blank"
				class="px-4 py-2 rounded-xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 hover:opacity-90 text-sm"
			>
				Join Discord
			</NuxtLink>
		</div>
