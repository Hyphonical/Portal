...
export default defineEventHandler(async (event): Promise<McStatusResponse> => {
  ...
  if (!host) {
    return { online: false, host: null, port, players: { online: 0, max: 0 }, version: null, motd: null };
  }

  try {
    const url = `https://api.mcstatus.io/v2/status/java/${host}:${port}`;
    const res: McApiResponse = await $fetch<McApiResponse>(url, { timeout: 5000 });
    ...
    return { online: false, host, port, players: { online: 0, max: 0 }, version: null, motd: null };
  }
});